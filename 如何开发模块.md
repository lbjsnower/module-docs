...
在`工具栏`中点击`插入代码工具`，弹出`工具列表栏`，找到`超参数调参工具`后点击`插入代码`。然后该工具的示例代码将自动插入到您的 Notebook中 ，同时 mo 工具包将自动导入到你的项目中，你可在左侧的文件列表中找到 mo 文件夹。

具体操作请见下图：


<img src='http://mo-imgs.momodel.cn/hyperparameter.gif' width=60% height=40%>
...
- 惩罚系数C：用来平衡分类间隔margin和错分样本的，default C = 1.0；
- 核函数 kernel：参数选择有RBF, Linear, Poly, Sigmoid, 默认的是"RBF";
- 核函数参数gamma：核函数的系数('Poly', 'RBF' and 'Sigmoid'), 默认是gamma = 1 / n_features。

4.2 定义调参的参数信息

tunables 可选的参数类型为 INT、 INT_EXP、 INT_CAT、 FLOAT、 FLOAT_EXP、FLOAT_CAT和 STRING, BOOL。
...
注意:
- 参数trail_numer，表示试验的次数，一般情况下设置大一点比较好，比如10次等。
- 因为调参可能需要花费较长的时间，建议创建 job 来进行模型的调参，并在 job 日志中查看调参的打印输出，更多关于 job 的信息，可参考这里。
